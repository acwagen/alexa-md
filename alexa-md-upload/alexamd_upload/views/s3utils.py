...
def s3validate(s3_bucket=S3_BUCKET):
...
def s3upload(filename, file, s3_bucket=S3_BUCKET):
...
def s3delete(ids, s3_bucket=S3_BUCKET):
...
def s3fetch(id, s3_bucket=S3_BUCKET):
